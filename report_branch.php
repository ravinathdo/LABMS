    ...
    <body>
        ...
        <div class="row" style="padding: 5px">
            ...
            <div class="col-md-10">
                ...
                <form class="form-inline" action="report_branch.php" method="post">
                           ...
                           $resultPost =  getData($sql);
                            if ($resultPost != FALSE) {
                                while ( $row = mysqli_fetch_assoc($resultPost)) {
                                    ?> <option value="<?= $row['id'] ?>"><?= $row['branch_name'] ?></option> <?php
                                }
                            }
                            ?>
                ...
                if (isset($_POST['btnReport'])) {
                    ...
                    $sql = "SELECT ld_invoice.*,lb_branch.branch_name FROM ld_invoice 
INNER JOIN lb_branch ON lb_branch.id = ld_invoice.branch_id
WHERE ld_invoice.branch_id = '$branch_id' AND DATE(ld_invoice.invoice_datetime) >= '$from_date' AND DATE(ld_invoice.invoice_datetime) <= '$to_date'";
                    $resultPost = getData($sql);
                    ?><br>
                    <center>
                        <h3>Branch Report</h3>
